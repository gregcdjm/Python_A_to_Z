...
class SuperPlayer:
        def __init__(self, pseudo , health, attack): # <--- dépôt des informations du joueur
            self.pseudo = pseudo
            self.health = health                     # <--- création et stockage des carastérisitques 
            self.attack = attack                     # du joueur selon les informations du dépot.

            # Petit message de bienvenue pour s'assurer sue le joueur est bien enregistré.
            print("Bienvenue au joueur "+self.pseudo+" vous avez ",str(health),"point de vie et {} "
